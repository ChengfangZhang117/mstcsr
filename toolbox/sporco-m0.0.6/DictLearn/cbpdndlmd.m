...
%
%
% Author: Brendt Wohlberg <brendt@lanl.gov>  Modified: 2017-02-01
%
...
while k <= opt.MaxMainIter && (rx > eprix|sx > eduax|rd > eprid|sd >eduad),
  ...
  Yf = fft2(Y);
  YSf = sum(bsxfun(@times, conj(Yf(:,:,1:(end-1),:)), Sf), 4);
  ...
  if strcmp(opt.LinSolve, 'SM'),
    Df = solvemdbi_ism(Yf(:,:,1:(end-1),:), sigma, YSf + sigma*fft2(G - H));
  else
    [Df, cgst] = solvemdbi_cg(Yf(:,:,1:(end-1),:), sigma, ...
                      YSf + sigma*fft2(G - H), cgt, opt.MaxCGIter, Df(:));
  end
  clear YSf;
  ...
  if opt.StdResiduals,
    ...
    eprid = sqrt(Ng)*opt.AbsStopTol+max(nD,nG)*opt.RelStopTol;
    eduad = sqrt(Nd)*opt.AbsStopTol+sigma*nH*opt.RelStopTol;
  else
    % See wohlberg-2015-adaptive
    rd = norm(vec(AD - G))/max(nD,nG);
